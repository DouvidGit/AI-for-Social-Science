# 大家都排那边 — MVP Spec

## 它做什么    (一句话)
一个 1 分钟完成的线上排队选择实验：用户在三条都可用的队伍中选择一条，并留下选择理由。

## 给谁用       (一个具体用户)
一位高中同学，愿意用手机或电脑快速体验一个校园活动入口的模拟选择。

## 核心流程     (用户走的 3–5 步)
1. 用户打开页面，看到校园活动入口的简短说明。
2. 用户看到三条队伍，其中一条明显人更多，三条都可进入同一活动。
3. 用户点击选择自己想排的队伍。
4. 用户回答两个问题：为什么选这条队，是否注意到某条队人更多。
5. 页面显示感谢参与，并提示反馈已记录。

## 不在 MVP 里  (「以后再说」清单)
- 倒计时和时间压力实验。
- 复杂动画、镜头推进和卡通人物行走。
- 人数均衡对照组和多条件随机实验。
- 等待时间提示、最快队伍提示或其他 nudge 类型。
- 年龄、性别、邮箱订阅等额外信息收集。
- 数据看板、论文分析页面和 AI nudge generator。
- 课堂提问、感谢表达、演讲反馈等其他生活场景。

## 怎么收集反馈
记录用户选择的队伍、哪条队是人多队、用户是否注意到人数差异，以及用户填写的一句话理由。第一版只验证用户是否理解场景、愿不愿意完成、理由中是否自然出现从众相关线索。

## Research Question & Hypothesis
Research Question: How do differences in visible queue length in an interactive online queue simulation affect high school students' line choice?

Hypothesis: I hypothesize that when one line is visibly more crowded while all other factors are identical, high school students will be more likely to choose the crowded line over shorter lines because they treat others' choices as a signal of reliability.
